...
let StepIsSafe (firstReading:Level) (secondReading:Level) (direction:ChangeDirection) : bool =
    ...
    if firstReading = secondReading then
        ...
        if direction = Increasing && secondReading < firstReading then
            safe <- false
    ...
    if firstTriplet[0] = firstTriplet[1] then
        ...
        if DifferenceIsSafe firstTriplet[0] firstTriplet[1] then
            ...
            if firstTriplet[1] = firstTriplet[2] then
                ...
                direction <- Decreasing
    printfn $"After considering first triplet, report is safe: '%A{reportIsSafe}', direction is '%A{direction}', number of ignore readings is '%A{ignoredReadings}'"
...
let reportStatusesWithinTolerance = Seq.map (fun report -> ReportIsSafeWithBadnessToleranceBruteForce report) parsedReports
